...
//Same as like post but set is_liked to 0
 ...
 if($is_liked){ //if list is not empty
    ...
    //happens only after liking the post
    $query = $mysqli->prepare("UPDATE likes SET Is_liked= 0 WHERE User_id=? && Image_id=?");
    $query->bind_param("ii", $User_id, $Image_id);
    $query->execute();
    ...
    $query = $mysqli->prepare("INSERT INTO likes( User_id, Image_id, Is_liked) VALUES ( ?, ?, 0)");
    $query->bind_param("ii", $User_id, $Image_id);
    $query->execute();
